...
class RMR(TDAgent):
    ...
    def decide_weights(self, x, prev_weights):
        ...
        b = self.update(data_close, self.history, prev_weights, self.eps, self.W)
        return b

    def update(self, data_close, data, prev_weights, eps, W):
        t1 = data.shape[0]
        if t1 < W+2:
            x_t1 = data[t1-1, :]
        else:
        ...
        if np.linalg.norm(x_t1 - x_t1.mean())**2 == 0:
            ...
            tao = min(0, (x_t1.dot(prev_weights)-eps) / np.linalg.norm(x_t1 - x_t1.mean())**2)
        if self.b is None:
