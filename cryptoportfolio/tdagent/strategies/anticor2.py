...
class ANTICOR2(TDAgent):
    ...
    '''
    def __init__(self, lookback_window_size=30, exp_w=None, data_day=None):
        super(ANTICOR2, self).__init__()
        self._lookback_window_size = lookback_window_size
        self.exp_ret = np.ones((self._lookback_window_size-1,1))
        self.exp_w = exp_w
        self.exp_ret2 = np.ones((self._lookback_window_size-1,1))
        self.exp_w2 = np.ones((self._lookback_window_size-1, self._lookback_window_size-1)) / (self._lookback_window_size-1)
    ...
    def decide_weights(self, x, prev_weights=None):
        ...
        if self.exp_w is None:
            self.exp_w = np.ones((self._lookback_window_size-1,m)) / m

        if self.data_day is None:
            self.data_day = np.zeros((1,self._lookback_window_size-1))
            mid = np.dot(self.history[-1,:], self.exp_w.T)
            self.data_day = mid[None,...]

        for k in np.arange(1,self._lookback_window_size):
