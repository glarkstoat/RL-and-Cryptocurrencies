...
from cryptoportfolio.tools.trading_utils import calculate_reward, plot_transaction_costs, transaction_factor, portfolio_value
...
class RLAgent(gym.Env):
    ...
    def __init__(
        ...
        self._lookback_window_size : int = lookback_window_size
        self._features = features
        self._feature_number : int = len(self._features)
        self._commission_rate : float = commission_rate
        self._batch_size = 0
        self._weight_storage = []
        self._name = "DDPG"
    ...
    def step(self, action):
        ...
        # Manual application of softmax function
        action = F.softmax(torch.from_numpy(action), dim=0).numpy()#; print("action after softmax:", action)
    ...
    def initial_weights(self):
        """ Returns uniformly balanced weights were the capital is equally distributed 
    ...
    def set_weight_storage(self, buffer_size):
        self._weight_storage = np.empty((buffer_size + 1, self._number_of_coins + 1))   
        self._weight_storage[0] = self.initial_weights()
        
    def overwrite_weight_storage(self, pos):
        last_action = self._PVM[-1]
        self._weight_storage[pos+1] = last_action
